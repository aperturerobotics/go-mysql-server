...
func (c *Concat) IsNullable(ctx *sql.Context) bool {
...
func (c *Concat) Eval(ctx *sql.Context, row sql.Row) (any, error) {
	var parts []string
	...
	for _, arg := range c.args {
		...
		content, _, err := types.ConvertToCollatedString(ctx, val, arg.Type(ctx))
		if err != nil {
			return nil, err
		}
