...
By default, `go-mysql-server` builds against a non-compatible regex
implementation based on the Go standard library `regex.Regex`. This avoids a
native ICU dependency in the default build.

To opt into ICU-compatible regexes, `go-mysql-server` has a dependency on
...
which depends on `go-mysql-server` with the ICU-backed regex engine, you should
have a C/C++ toolchain, build with Cgo enabled, install libicu-dev or the
equivalent for your environment, and compile with `-tags=gms_icu_regex`.

The pure-Go regex backend also remains selectable via `-tags=gms_pure_go`, and
`purego` builds also use the pure-Go backend automatically. Please note that
some of `go-mysql-server`'s tests do not pass with the pure-Go regex backend
and in general it is not recommended for users seeking full MySQL regex
compatibility.
